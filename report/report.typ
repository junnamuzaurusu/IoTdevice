...
まずは5日目までの学習成果を利用してESP OneボードからTCP接続によって工学部2号館にあるサーバーに接続した。このサーバーはリレーサーバーとして利用する。

その後、リレーサーバーからローカル環境のバックエンドサーバーに構築したデータベースへ観測データをHTTP通信によって送信した。ただし、おそらくリレーサーバーがセキュリティの観点からhttpによる通信を許可していないため、HTTPS規格での通信を行う必要があった。そのため、一度ngrokというAPIを使用した。本郷のサーバーからHTTPS通信を行ってローカルのngrokサーバーに接続し、リダイレクトすることでバックエンドサーバーに接続することができた。さらにバックエンドではIsolation Forestを用いて外れ値を検出し、最新のデータが外れ値かどうかを判定し、その結果もデータベースに登録した。
...
コード全体を参照されたい場合は以下のgithubを参照していただきたい。また、可視化されたデータのアニメーションについての動画もgithubのREADMEに記載されているため、そちらを参照していただきたい。
url: https://github.com/junnamuzaurusu/IoTdevice

== リレーサーバー
リレーサーバーではサーバー上にCSVファイルとしてセンサーからのデータを記録することに加え、json形式のデータをバックエンドサーバーに送信する役割を追加した。

== バックエンドサーバー
バックエンドのサーバーはFastAPIを使用して構築した。
